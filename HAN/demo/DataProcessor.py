...
class DataProcessor():
	...
	def preTreatMent(self):
		...
		self.labelTypes = list(set(corpusLabels)) # 获得所有标签类型
		...
		self.word2idx = dict(zip(vocab, range(self.vocSize)))
		print('单词表大小为:{}'.format(self.vocSize))
	...
	def train(self, index):
		...
		@return: {
					trainInput: [语句数量, seqLen] dtype = torch.LongTensor
					trainTarget: [语句数量] dtype = torch.LongTensor
				}
		'''
		trainInput = []
		for i in index:
			temp = torch.zeros(self.seqLen).to(device)
		...
		trainTarget = []
		for i in index:
			temp = self.labelTypes.index(self.corpusLabels[i])
	...
	def test(self, index):
		...
		testTarget = []
		for i in index:
			temp = torch.zeros(len(self.labelTypes)).to(device)
			temp[self.labelTypes.index(self.corpusLabels[i])] = 1
